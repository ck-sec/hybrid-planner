...
test('public pages lead with hybrid training and room for other sports without sport-specific artwork', () => {
  ...
  assert.match(hero, /Fixed club training/)
  assert.match(hero, /Practice stays in the same week/)
  assert.match(home.html, /Add your usual club training or activities as fixed sessions/)
  ...
  assert.match(hero, /Saved in this browser\. Share only when you choose\./)
  assert.match(hero, /href="\.\/app\/">Open planner/)
  assert.match(home.description, /browser-local/)
  assert.match(home.description, /copy\/paste JSON review/)
  ...
  assert.match(home.html, /"description":"A free, MIT-licensed, browser-local training planner[^"]*aerobic, strength, mobility and fixed club sessions[^"]*vendor-neutral AI copy\/paste JSON loop/)
})
...
test('setup separates desired and current training and distinguishes AI advice from integrity checks', () => {
  ...
  assert.match(home, /Add club training or other recurring sessions as fixed work with days, start time and duration before AI/)
  assert.match(home, /Confirm current training separately from your desired routine/)
  assert.match(home, /Review a manual draft or pasted AI week, then approve it after app checks/)
  assert.match(home, /Copy, discuss, review/)
  assert.match(home, /Direct API support is future work/)
  assert.match(home, /AI can draft a week\. The app checks it\. You approve\./)
  ...
  assert.match(method, /AI cannot redefine exercise profiles, approvals or initial weights/)
  assert.match(home, /You and your AI choose training frequency, rest and progression/)
  assert.match(home, /There is no built-in provider connection today/)
  assert.match(home, /not medical clearance or a guarantee against injury/)
  assert.match(home, /manual planning still works fully without AI/)
  assert.match(method, /historical prescriptions and weight records stay intact/)
  assert.match(method, /Manual planning, custom exercises and weekly review are complete without AI/)
  ...
  assert.match(method, /Saved weeks keep their own history/)
  ...
  assert.doesNotMatch(method, /It cannot prescribe|AI cannot supply its own numerical prescriptions|revisions affect next week only|version-[123]|<code>customExercises<\/code>|compatible API|API key/)
})
...
test('the landing page explains chat friction before introducing the setup workflow', () => {
  ...
  assert.ok(ownership)
  assert.match(ownership, /AI chats still need a connection/)
  assert.match(ownership, /Open your week, keep exercise cards nearby, and record what actually happened in the same browser profile/)
...
test('privacy describes local ownership without claiming all data can never leave the device', () => {
  ...
  for (const html of [home, privacy]) {
    ...
    assert.match(html, /[Rr]emote AI[^]*(?:connectivity|connection)|AI chats (?:still need a connection|require connectivity)/)
  ...
  assert.match(privacy, /Prior-plan archives are not exported/)
  ...
  assert.match(privacy, /[Cc]opying alone contacts no provider/)
  assert.match(privacy, /Opening a provider link visits that provider but does not send the brief automatically/)
  assert.match(privacy, /[Cc]opy it into your own AI chat and paste the final JSON reply back/)
  ...
  assert.match(privacy, /This MVP does not ask for provider API keys or send requests through a Hybrid Coach proxy/)
  assert.match(privacy, /Direct API integration is future work/)
  assert.match(privacy, /does not upload that history or create a remote backup/)
  assert.doesNotMatch(privacy, /excludes activity files, training logs and API keys|no logs (?:are|ever) sent|authorization header|compatible endpoint/i)
})
...
test('the planner entry is private metadata and mounts the local React app', async () => {
  ...
  assert.doesNotMatch(html, /being rebuilt|previous app is no longer available/)
...
test('marketing pages no longer carry the rebuild availability notice', () => {
  for (const page of marketingPages()) {
    assert.doesNotMatch(page.html, /aria-label="Planner availability"/)
    assert.doesNotMatch(page.html, /being rebuilt|app is unavailable|previous version/i)
  }
})
...
test('Cloudflare sends old app routes to the rebuild page without redirecting the destination', async () => {
  const redirects = await readFile(new URL('../public/_redirects', import.meta.url), 'utf8')
  assert.match(redirects, /^\/app \s*\/app\/ 301$/m)
  assert.match(redirects, /^\/app\/index\.html \s*\/app\/ 301$/m)
  assert.match(redirects, /^\/app\/:path \s*\/app\/ 302$/m)
  assert.match(redirects, /^\/app\/:path\/\* \s*\/app\/ 302$/m)
