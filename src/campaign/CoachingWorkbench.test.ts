...
test('workspace offers a key-free chat roundtrip and does not connect on render', async t => {
  ...
  try {
    ...
    assert.ok(sharing)
    assert.match(sharing, /more than seven movements/)
    ...
    assert.match(fullWeek, /Include my reviewed activity records and summary/)
    assert.match(fullWeek, /No titles, locations or original file\. Gaps remain unknown/)
