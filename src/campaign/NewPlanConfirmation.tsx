...
export default function NewPlanConfirmation({ state, ready, onBackup, onCancel, onConfirm }: {
  ...
  return <section className="cf-stack" aria-labelledby="new-plan-title">
    <div><h2 id="new-plan-title">Replace this plan?</h2></div>
    <p>This ends <strong>{state.draft.goalLabel}</strong> and removes its remaining workouts from your active calendar.</p>
    <p>Your {logs.length} recorded session{logs.length === 1 ? '' : 's'} stay in read-only history, including partial workouts and skips. Unlogged workouts stay unlogged.</p>
    <p className="cf-small">Edit your prefilled setup and confirm your current training before building a new plan.</p>
    {Object.keys(state.setDrafts).length > 0 && <p className="cf-small">Unsubmitted entries stay in the old plan's backup, not its logged training.</p>}
