...
export default function WorkoutCards({ cards, resources, onChange, exerciseId, readOnly = false, program }: WorkoutCardsProps) {
  ...
  return <section className="cf-notebook" aria-labelledby={`${notebookId}-title`}>
    <div className="cf-notebook-heading">
      <div><h3 id={`${notebookId}-title`}>Notes</h3></div>
      {!readOnly && <button type="button" className="cf-notebook-button" disabled={cards.length >= MAX_WORKOUT_CARDS || editor !== null} aria-expanded={editor !== null} aria-controls={`${notebookId}-editor`} onClick={addCard}>Add note</button>}
    </div>
    <p className="cf-notebook-disclaimer">Reference only: notes do not add work or change your plan.</p>
    {exerciseId !== undefined && <p className="cf-notebook-meta">Notes for {catalog.find(exercise => exercise.id === exerciseId)?.name ?? 'this exercise'}.</p>}
    ...
    <div className="cf-notebook-list">{visible.map(card => {
      ...
      return <article key={card.id} className={`cf-notebook-card${unavailable ? ' cf-notebook-unavailable' : ''}`} aria-labelledby={`${notebookId}-${card.id}`}>
        ...
        <p className="cf-notebook-identity">{exercise ? <><span>Exercise: </span><strong>{exercise.name}</strong></> : <strong>Unscheduled drill idea</strong>}</p>
        {card.resources.length > 0 && <p className="cf-notebook-meta">Equipment: {resourceLabels(card.resources).join(', ')}.</p>}
        {unavailable && <p className="cf-notebook-warning">Unavailable for use. Missing: {missing.length ? labelsFor(missing) : 'compatible exercise equipment'}. Kept as a draft.</p>}
        ...
        {!readOnly && <details className="cf-notebook-text"><summary aria-label={`Manage ${card.title}`}>Manage note</summary><div className="cf-notebook-actions">
    ...
    {editor && <fieldset id={`${notebookId}-editor`} className="cf-notebook-editor" onKeyDown={event => {
      ...
      <legend>{editor.isNew ? 'New note' : 'Edit note'}</legend>
      <p className="cf-notebook-meta">{sourceLabel(editor.card)}. Editing does not approve a draft.</p>
      <label>Title<input type="text" value={editor.card.title} maxLength={WORKOUT_CARD_LIMITS.title} placeholder="Your note title" onChange={event => patchCard({ title: event.target.value })} /></label>
      ...
      <p className="cf-notebook-meta">To create or change an exercise, use the exercise editor instead.</p>
      ...
      <fieldset className="cf-notebook-resources"><legend>Equipment needed</legend><div>{resourceOptions.map(resource => <label key={resource}>
      ...
      {editorUnavailable && <p className="cf-notebook-warning">Missing: {editorMissing.length ? labelsFor(editorMissing) : 'compatible exercise equipment'}. This can only be saved as a draft.</p>}
