...
test('restored history reaches handoff only through explicit confirmed data-only scope, never as an inferred baseline', t => {
  ...
  assert.equal(context.trainingHistory.recordCount, 3)
  assert.deepEqual(context.trainingHistory.recordedRange, brief.context.trainingHistory.summary.dateRange)
  assert.equal(context.trainingHistory.stale, true)
  assert.deepEqual(context.trainingHistory.recentWindow.byType, [])
  assert.equal(Object.hasOwn(context.trainingHistory, 'records'), false)
  assert.match(context.trainingHistory.detail, /individual rows omitted/)
  assert.ok(exported.length < 16000)
