...
import { recommendedExercises, recommendationForExercise } from '../../engine/recommendations.ts'
...
const steps = ['Welcome', 'Your direction', 'Your rhythm', 'Your base', 'Your week', 'Ready to start']
const equipmentOptions: { label: string; description: string; equipment: Equipment[] }[] = [
  { label: 'Bodyweight', description: 'No kit needed', equipment: ['bodyweight'] },
  { label: 'Dumbbells', description: 'A simple home setup', equipment: ['bodyweight', 'dumbbell'] },
  { label: 'Gym', description: 'Barbells, dumbbells & bands', equipment: ['bodyweight', 'dumbbell', 'barbell', 'bands'] },
]
...
export default function CampaignSetup({ state, update, onAI, connected, onDisconnect }: {
  ...
  if (state.step === 0) return <section className="cf-welcome">
    <div className="cf-welcome-copy">
      ...
      <p className="cf-lead">Start with a classic run + lift plan.<br />Or connect AI and tell us what you're building toward.</p>
      <button className="cf-button cf-primary cf-hero-cta" onClick={() => update(previous => ({ ...prepareRecommendedSetup(previous), step: 1 }))}>Find my starting point <Icon name="arrow" /></button>
  ...
  return <div className="cf-setup-layout">
    <aside className="cf-setup-aside"><div><p className="cf-kicker">MAKE IT YOURS</p><h2>A strong base.<br />Your own direction.</h2></div><ol>{steps.slice(1).map((step, index) => <li key={step} aria-current={state.step === index + 1 ? 'step' : undefined}><span>{state.step > index + 1 ? <Icon name="check" size={16} /> : `0${index + 1}`}</span>{step}</li>)}</ol><p className="cf-small">No training spreadsheet needed.<br />Your answers stay on this device.</p></aside>
    <form className="cf-setup-form" onSubmit={forward}>
      <div className="cf-step-header"><button type="button" className="cf-icon-button" aria-label="Previous step" onClick={() => { setIssue(''); update(previous => ({ ...prepareRecommendedSetup(previous), step: previous.step - 1 })) }}><Icon name="back" /></button><span>0{state.step} <span className="cf-muted">/ 05</span></span><span>{steps[state.step]}</span></div>
      <div className="cf-step-progress" aria-hidden="true">{steps.slice(1).map((step, index) => <i key={step} className={state.step > index ? 'cf-filled' : ''} />)}</div>
      ...
      {state.step === 3 && <section className="cf-stack">
        ...
        <fieldset className="cf-equipment-options"><legend>What can you train with?</legend><div>{equipmentOptions.map(option => {
          const selected = option.equipment.length === draft.equipment.length && option.equipment.every(item => draft.equipment.includes(item))
          return <button type="button" key={option.label} aria-pressed={selected} onClick={() => { if (!selected) { setSwapping(null); patch({ equipment: [...option.equipment] }, { exerciseIds: [...recommendedExercises(option.equipment)] }) } }}><strong>{option.label}</strong><span>{option.description}</span></button>
        })}</div></fieldset>
        <div className="cf-recommendation-list">{setup.exerciseIds.map((id, index) => {
          ...
          const alternatives = DEFAULT_LIBRARY.exercises.filter(item => !item.highSkill && item.pattern === exercise.pattern && !setup.exerciseIds.includes(item.id) && item.equipment.every(piece => piece === 'none' || draft.equipment.includes(piece)))
        ...
        <button type="button" className="cf-ai-teaser" onClick={() => {
          if (setup.mode !== 'assisted') patch({}, { mode: 'assisted', goalText: setup.goalText || 'Help me tailor a classic running and lifting routine using my available equipment.' })
          onAI()
        }}><Icon name="spark" /><div><strong>Add or change exercises with AI</strong><span>Describe your goal or preferences. Review the proposed cards before applying.</span></div><Icon name="chevron" size={18} /></button>
