...
function layout(path: string, title: string, description: string, body: string, options: {
  ...
  if (path === '/') graph.push({
    ...
    description: 'A free, MIT-licensed, browser-local training planner for aerobic, strength, mobility and fixed club sessions. Build and review your week manually, then use a vendor-neutral AI copy/paste JSON loop only when you want help.',
  })
...
function cta(root: string): string {
  return `<section class="end-cta wrap"><div><p class="eyebrow">Your training. Your terms.</p><h2>A plan you own.</h2><p>Your routine. Your records. AI only if you choose.</p></div>
    <div><a class="button button-cream" href="${root}app/">Open planner ${arrow}</a><p class="fine-print">Free. No login, subscription or provider lock-in.</p></div>
  </section>`
}
...
function home(): string {
  ...
  return `<section class="hero wrap">
    <div class="hero-copy"><p class="eyebrow"><span class="short-line"></span> Browser-local. Open source. Free.</p>
      <h1>Running<br>and lifting.<br><span class="serif">One plan that fits.</span></h1>
      <p class="hero-description">And room for the other sports you love. Plan aerobic work, strength, mobility and fixed club training together, around your equipment, commitments and recovery.</p>
      ...
      <p class="hero-promise">No account. No subscription. No telemetry.<br>Saved in this browser. Share only when you choose.</p>
    </div>
    <figure class="week-figure">
      ...
      <div class="week-board"><div class="board-heading"><div><span class="eyebrow">Your training space</span><h2>One week. Your way.</h2></div><span class="local-tag">Local</span></div>
        ...
        <ul class="sample-week">
          ...
          <li class="sample-day sport"><span class="day-dot" aria-hidden="true"></span><span><strong>Fixed club training</strong><small>Practice stays in the same week</small></span></li>
  ...
  <section class="ownership-section wrap">
    <div><p class="eyebrow">At the gym, not in the chat</p><h2>Same browser.<br><span class="serif">Still your session.</span></h2><p>Open your week, keep exercise cards nearby, and record what actually happened in the same browser profile.</p><p class="fine-print">AI chats still need a connection.</p></div>
  ...
  <section class="how-section" id="how-it-works"><div class="wrap how-grid">
    ...
    <ol class="steps">
      ...
      <li><span aria-hidden="true">02</span><div><h3>Routine</h3><p>Choose aerobic, strength, mobility, equipment and space. Add club training or other recurring sessions as fixed work with days, start time and duration before AI.</p></div></li>
      <li><span aria-hidden="true">03</span><div><h3>Review</h3><p>Confirm current training separately from your desired routine. Review a manual draft or pasted AI week, then approve it after app checks.</p></div></li>
    </ol>
  </div></section>
  ...
  <section class="section harness-section wrap">
    ...
    <div class="harness-grid"><div class="harness-option"><h3>Copy, discuss, review</h3><p>Preview a setup or weekly-review brief. Copy it into <a href="https://chatgpt.com">ChatGPT</a>, <a href="https://claude.ai">Claude</a> or <a href="https://duck.ai">Duck.ai</a>, or another chat that can return planner JSON, discuss it, then paste the final JSON reply here for review. Nothing is imported from your chat automatically.</p></div><div class="harness-option"><h3>Direct API support is future work</h3><p>The current MVP keeps AI vendor-neutral with copy and paste. There is no built-in provider connection today, and manual planning still works fully without AI.</p></div></div>
    <div class="engine-boundary"><strong>AI can draft a week. The app checks it. You approve.</strong><p>You and your AI choose training frequency, rest and progression. The app flags training concerns and checks data, equipment and recorded-work integrity. Passing checks is not medical clearance or a guarantee against injury.</p></div>
    <p class="fine-print">Nothing is sent automatically. Provider accounts, free usage limits and privacy policies vary; AI chats need connectivity and may cost money. Direct API integration is future work. <a href="./privacy/">See exactly what you share</a>.</p>
  </section>
  ...
  <section class="section wrap faq-section"><div><p class="eyebrow">Good questions</p><h2>Clear choices.<br>Honest limits.</h2><p>An early, open project. Here&rsquo;s what to know before you start.</p></div><div class="faq-list">
    ${[
      ['Is Hybrid Coach really free?', 'The planner is free and MIT licensed. There is no trial, subscription, or account. Optional third-party AI chats are separate and may charge.'],
      ['Can my other sports fit into the plan?', 'Yes. Add your usual club training or activities as fixed sessions alongside aerobic, strength and mobility work, with their days, start time and duration. Keep them in the same week rather than juggling separate schedules. This organises your training around those sessions; it does not generate a complete sport-specific coaching programme for every activity.'],
      ['Do I need AI to use it?', 'No. Manual planning, custom exercises, logging, and weekly review work without AI. Optional AI currently uses a vendor-neutral copy/paste JSON loop; direct API integration is future work. Training concerns are advice, not automatic vetoes. You review and approve each week. Data and equipment checks still apply.'],
      ['What would I share with AI?', 'Only the brief you explicitly choose to share. A setup brief contains your goal, desired routine, confirmed current-training facts when available, equipment, and planning context, not campaign session logs. A weekly-review brief also includes that week&rsquo;s actual logs, feedback, notes, and recorded health flags. A separate default-off choice includes confirmed normalized Garmin records and their summary. Raw activity files, activity titles and locations, and prior-plan archives are not included. Preview the brief before sharing; your provider&rsquo;s policies apply.'],
      ['Can I use it offline?', 'The planner is browser-local. Reopen /app/ in the same browser to return to your saved plans, library and logs. AI chats still need connectivity, and direct API integration is future work.'],
      ...
      ['Is this a personal coach or a beginner lifting course?', 'No. It does not teach lifting technique, diagnose pain, measure readiness, or promise a race peak. It helps you organise an established routine. AI training choices are yours to review; planning notes and app checks cannot guarantee injury prevention.'],
...
function method(): string {
  return `<section class="page-intro wrap"><p class="eyebrow">The thinking, in the open</p><h1>A planner you<br>can <span class="serif">look inside.</span></h1><p>An open-source, browser-local planner for people who run and lift, with product choices you can inspect.</p></section>
    <div class="document-layout wrap"><aside><p class="eyebrow">Our approach</p><a class="text-link" href="${sourceUrl}">Read the source ${arrow}</a><a class="text-link" href="../learn/">Guides ${arrow}</a></aside><div class="prose">
      <section><h2>One week at a time, with a reason.</h2><p>This MVP keeps planning browser-local. Set your goal, preferred weekly mix, equipment and fixed club sessions, then build one week at a time. Manual planning and AI-drafted JSON both start from context you can review before approval.</p><p>A block gives the broad shape; sessions are built one week at a time. Desired training is not a baseline. Frequency and average-duration sliders show the weekly time you want; a long run can exceed that average. Confirm current training separately. Imported records cannot fill those answers automatically.</p></section>
      <section><h2>Planning notes, not a readiness score.</h2><p>The planner keeps aerobic, strength and mobility work explicit instead of pretending to calculate readiness. It does not produce performance predictions or injury-risk calculations.</p><p>There is no acute-to-chronic workload ratio, readiness gauge or learned physiological fatigue model. You keep the notes, skipped-work reasons and review context visible for yourself and any AI you choose to use.</p><p><a href="${sourceUrl}">Inspect the current source and contracts</a>.</p></section>
      <section><h2>Your training choice. Reliable records.</h2><p>For manual edits and new AI weeks, frequency, volume, rest and recovery checks are advice, not automatic vetoes. Two-a-days and weeks without a full rest day are supported. Pain and health reports remain visible; passing data checks is not medical clearance.</p><p>Malformed data, unavailable equipment and changes to recorded work remain blocked. Fixed commitments cannot be rewritten for convenience. Saved weeks keep their own history.</p></section>
      <section><h2>Custom exercises, reliable identities.</h2><p>Choose library exercises or create custom cards manually, then use them in manual or AI-assisted weeks. Cards describe the movement, focus, purpose and equipment. Supported strength, mobility and controlled target throwing use immutable identities and app-owned profiles. A complete authored week may use up to 32 distinct exercise and drill identities. That maximum is not a recommended dose.</p><p>Review technique and fit before approval; app checks are not technique validation or medical clearance. Changed custom techniques need new identities; historical prescriptions and weight records stay intact. A midweek Swap can replace supported remaining lifting or mobility work, preserving recorded sets and the original prescription history, without borrowing working weights. It defaults to this session; a future preference is recorded for later proposals, not automatically applied.</p><p>Reference notes cannot add scheduled work. Renaming unsupported work cannot bypass checks. Current setup has no separate rower/SkiErg baseline form; saved modality-specific conditioning baselines remain supported and require matching equipment.</p></section>
      <section><h2>AI can draft a week. The app checks it.</h2><p>Setup is Goal, Routine, then Review. Enter desired training, equipment and fixed-session days, start time and duration before AI. Confirm current training separately. At setup or weekly review, copy the previewed brief into your chat, discuss it, then paste the final JSON reply for review.</p><p>AI can propose complete weeks with supported sets, repetitions, seconds, effort targets, duration and placement. You and your AI choose progression, with training concerns surfaced for review rather than silently reducing the week. AI cannot redefine exercise profiles, approvals or initial weights, or rewrite existing observations and performed work. Technical format bounds and equipment checks still apply. You approve each proposal.</p><p>There is no bundled model or hidden AI service. The current MVP keeps provider choice vendor-neutral through copy and paste; direct API integration is future work. Manual planning, custom exercises and weekly review are complete without AI.</p></section>
      ...
      <section><h2>Built for real interruptions.</h2><p>Time skips do not imply fatigue. Fatigue skips inform the next review. Deletion records a change, not fatigue. AI replies and exercise approvals cannot erase health reports or performed work.</p><p>No streaks or badges: logs record what happened, not what you owe the plan.</p></section>
      ...
      <section><h2>Evidence and accountability.</h2><p>Guides distinguish research findings from practical examples. Editorial responsibility sits with the Hybrid Coach project, which claims no clinical review, professional coaching certification or scientific validation of its planning flow.</p><p>Inspect the code, discuss rules or report factual errors in <a href="${sourceUrl}/issues">public GitHub issues</a>. Do not post private training records, health details or pasted handoff JSON. The code is available under the <a href="${sourceUrl}/blob/main/LICENSE">MIT license</a>.</p></section>
    </div></div>${cta('../')}`
}
...
function privacy(): string {
  return `<section class="page-intro wrap"><p class="eyebrow">Local first, in plain language</p><h1>Your data.<br><span class="serif">Your choice to share.</span></h1><p>What stays in your browser, what the host sees, and what you choose to share.</p></section>
    <div class="document-layout wrap"><aside><p class="eyebrow">Privacy &amp; your data</p><p class="fine-print">Updated 7 September 2026</p></aside><div class="prose">
      ...
      <section><h2>AI is a separate, explicit connection.</h2><p>AI is optional at the final setup review and weekly review. Visiting, entering Goal or Routine, or opening a panel sends nothing to AI. Preview the brief, copy it into your own AI chat and paste the final JSON reply back. Copying alone contacts no provider. Opening a provider link visits that provider but does not send the brief automatically. Direct API integration is future work.</p><p>A setup brief includes your goal and date, desired routine, confirmed current-training facts when available, equipment and space, availability and fixed sessions, exercise selection, custom definitions, eligible catalog and profiles, reference cards and relevant planned sessions. Setup sharing does not include campaign session logs.</p><p><strong>A weekly-review brief additionally includes this week&rsquo;s actual training records:</strong> sets, repetitions, seconds, weights, duration, effort, notes and recorded health flags, plus easier/as-expected/harder feedback, optional distance and average heart rate, stopped-early outcomes and recorded changes. Partial work, skip reasons, removals, omissions and unlogged sessions stay distinct. Missing records remain unknown, not assumed completion. These records may be sensitive; preview them before sharing.</p><p>A separate default-off choice includes confirmed normalized Garmin records and their summary: activity type, local timestamp, timer duration, and optional distance, average heart rate, moving and elapsed time. Activity titles, locations and raw files are excluded. Prior-plan archives are not exported. The external provider or chat has its own terms and privacy policy.</p><p>This MVP does not ask for provider API keys or send requests through a Hybrid Coach proxy. Optional AI may cost money; manual planning, custom exercises and weekly review need no AI.</p></section>
      <section><h2>Browser-local in the same profile.</h2><p>The MVP saves your plans, library and logs in this browser profile. Reopen the planner in the same browser to continue with local records. AI chats require connectivity, and another browser or device needs an exported backup to see the same data.</p></section>
      <section><h2>Questions or corrections.</h2><p>The source and public issue tracker are on <a href="${sourceUrl}">GitHub</a>. Do not post private records or pasted handoff JSON in a public issue. This site has no contact form, mailing-list signup, or data-collection endpoint.</p></section>
    </div></div>`
}

export function marketingPages(): MarketingPage[] {
  const pages = [
    { path: '/', title: 'Free Hybrid Training Planner for Running & Lifting | Hybrid Coach', description: 'Running, lifting and your other sports in one plan. Free, open-source and browser-local, with manual planning, optional AI copy/paste JSON review, and training records you own.', body: home() },
    ...
    { path: '/method/', title: 'How Hybrid Coach Plans a Training Week', description: 'A browser-local planning flow, your exercise library, and optional AI copy/paste JSON review. See what stays manual, what AI can draft, and how training history is kept.', body: method() },
    { path: '/privacy/', title: 'Privacy & Your Local Training Data | Hybrid Coach', description: 'Local browser storage, backups you control, and optional AI copy/paste sharing. What the static host sees and what you choose to share. No accounts or telemetry.', body: privacy() },
  ]
  return [
